...
import { FundingRatePoint } from '@/types/market';

interface FundingRateChartProps {
  data: FundingRatePoint[];
}

export default function FundingRateChart({ data }: FundingRateChartProps) {
  ...
  useEffect(() => {
    ...
    const renderChart = () => {
      ...
      const xScale = (i: number) => padLeft + (i / Math.max(1, data.length - 1)) * chartW;
      const yScale = (rate: number) =>
        padTop + (1 - (rate - minRate) / rateRange) * chartH;
      ...
      if (typeof ctx.roundRect === 'function') {
        ...
        ctx.rect(w - padRight + 6, lastY - labelH / 2, labelW, labelH);
      }
      ctx.fill();
